    ...
    def get_bestshot(self):
        ...
        # 大本营有球
            ...
            if self.player_is_init == stone_is_init:
                # 判断是否存在对方球在保护球的前方
                for stone in stone_in_house:
                    if not stone[3]:  # 对方的球
                        if stone[1] < x:  # 对方球在保护球前方
                            # 判断保护球与对方球之间的距离
                            distance_between = self.get_dist(stone[1] - x, stone[2] - y)
                            if distance_between < 0.2:  # 距离较近，选择击飞对方球
                                v0 = 3.613 - 0.12234*y + 1
                                h0 = x - 2.375
                                shot_msg = "BESTSHOT " + str(v0) + " " + str(h0) + " 0"
                                return shot_msg

                # 对方球不在保护球前方或距离较远，选择保护己方球
                v0 = 3.0  # 保持较低的速度
                h0 = x - 2.375
                shot_msg = "BESTSHOT " + str(v0) + " " + str(h0) + " 0"
...
if __name__ == '__main__':
    ...
    key = "liuwenqing_abdd668c-9bae-4ee7-907d-dd7d1f1415fd"
    #初始化AI选手
    airobot = AIRobot(key)
    #启动AI选手处理和服务器的通讯
